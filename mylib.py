...
def hyperConvert2D(Image3D):
    if Image3D.ndim == 2:  # 检查输入张量的维度数是否为2，以确定输入数据的形式。
        numBands = 1
        h, w = Image3D.shape  # 如果输入张量是二维的，设置波段数为1，然后获取输入张量的高度 h 和宽度 w
    else:
        h, w, numBands = Image3D.shape  # 获取输入张量的高度 h、宽度 w 和波段数 numBands。

    Image2D = np.reshape(Image3D, (w * h, numBands)).T
    # 将输入的三维张量重塑为一个二维矩阵 Image2D。
    # 使用 reshape 函数将三维张量拉伸为一个二维矩阵，其中每列代表一个样本的波段数据。
    # 转置运算符 ' 用于将结果矩阵进行转置，以便每列对应一个样本的波段数据。(numBands, w*h)

    return Image2D


def hyperConvert3D(Image2D, h, w, numBands):
    numBands, N = Image2D.shape

    if N == 1:
        Image3D = np.reshape(Image2D, (h, w))
    else:
        Image3D = np.reshape(Image2D.T, (h, w, numBands))

    return Image3D

def hyperConvert3D_tensor(Image2D, h, w, numBands):
    numBands, N = Image2D.shape

    if N == 1:
        Image3D = torch.Tensor(Image2D).view(h, w)
    else:
        Image3D = torch.Tensor(Image2D.T).view(h, w, numBands)

    return Image3D

def hyperConvert2D_tensor(Image3D):
    if Image3D.ndim == 2:
        numBands = 1
        h, w = Image3D.shape
    else:
        h, w, numBands = Image3D.shape

    Image2D = torch.Tensor(np.reshape(Image3D, (w * h, numBands)).T)

    return Image2D

def Gaussian_downsample_tensor(z, fft_B, sf=4, s0=1):

    batch_size, L, M, N = z.shape
    x = torch.zeros((batch_size, L, M // sf, N // sf, L), dtype=z.dtype, device=z.device)

    for i in range(L):
        Hz = torch.ifft2(torch.fft.fft2(z[:, i, :, :]) * fft_B, signal_ndim=2)
        t = Hz[:, s0::sf, s0::sf]
        x[:, i, :, :] = t
...
class loss_func_RB(nn.Module):
    def __init__(self, R, B ):
        super(loss_func_RB, self).__init__()
        self.R = R
        self.B = B
    def forward(self, ref, out, pan, ms, sf=4):
        loss = nn.L1Loss()
        loss_ref = loss(ref, out)
        _, c, h, w = out.shape
        w1, h1 = self.B.shape
        # print('pc_2.shape:', pc_2.shape, mc_2.shape)# torch.Size([32, 32, 16, 16]) torch.Size([32, 32, 16, 16])
        coeff = torch.unsqueeze(self.B, 0)
        coeff = torch.unsqueeze(coeff, 0)
        coeff = torch.repeat_interleave(coeff, out.shape[1], 0)
        out_B = F.conv2d(out, coeff.cpu(), bias=None, stride=sf, padding=int((w1 - 1) / 2), groups=c) # .cuda

        RTZ = torch.tensordot(out, self.R, dims=([1], [1]))
        out_R = torch.Tensor.permute(RTZ, (0, 3, 1, 2))

        loss_pan = loss(pan, out_R)
        loss_ms = loss(ms, out_B)

        loss_total = loss_ref + 0.1 * loss_pan + 0.1 * loss_ms
        print('loss_ref:{} __loss_pan:{} __loss_ms:{} '.format(loss_ref, loss_pan, loss_ms))
        return loss_total
